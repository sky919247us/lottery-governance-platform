...
const GLOSSARY_DATA_ = [
  ...
  ['假日表', '農曆', 'lunar', '字串', '正月初一 / 二月十五 …', '注意：目前資料源(API)農曆不正確，待修'],
  ['假日表', '備註', 'note', '字串', '自由輸入', '排班視圖的備註列；可從前端直接編輯'],
  ...
  // ============= 預設排班 =============
  ['預設排班', '門市代號', 'store_id', '字串（FK）', 'LUCKY / BINGO / MONEY', '本店本班的固定員工'],
  ['預設排班', '班別代號', 'shift_code', '字串（FK）', 'MORNING / EVENING / NIGHT', ''],
  ['預設排班', '員工編號', 'employee_id', '字串（FK）', '對應「員工」', '排班視圖「載入預設」會整月填入此員工'],
  ['預設排班', '備註', 'note', '字串', '自由輸入', '可選填'],

  // ============= 留職停薪紀錄（Core）=============
  ['留職停薪紀錄', '留停ID', 'leave_id', '字串', 'L001、L002…', '主鍵'],
  ['留職停薪紀錄', '員工編號', 'employee_id', '字串（FK）', '', ''],
  ['留職停薪紀錄', '類型', 'type', '列舉', '育嬰留停 / 一般留停 / 病假留停 / 兵役 / 其他', ''],
  ['留職停薪紀錄', '起始日', 'start_date', '日期', 'YYYY-MM-DD', ''],
  ['留職停薪紀錄', '復職日', 'end_date', '日期', 'YYYY-MM-DD（空白=尚在留停）', ''],
  ['留職停薪紀錄', '扣除年資', 'deduct_tenure', '布林', 'TRUE / FALSE', 'TRUE=這段期間不算特休年資'],
  ['留職停薪紀錄', '核准人', 'approved_by', '字串', 'admin/店長 employee_id', ''],
  ['留職停薪紀錄', '核准時間', 'approved_at', '時間戳', '', ''],
  ['留職停薪紀錄', '備註', 'note', '字串', '自由輸入', ''],

  // ============= 請假紀錄（Attendance）=============
  ['請假紀錄', '請假ID', 'request_id', '字串', 'LR-流水號', '主鍵'],
  ['請假紀錄', '員工編號', 'employee_id', '字串（FK）', '', ''],
  ['請假紀錄', '假別', 'type', '列舉', '特休 / 事假 / 病假 / 公假 / 婚假 / 喪假 / 產假 / 其他', ''],
  ['請假紀錄', '起始日', 'start_date', '日期', 'YYYY-MM-DD', ''],
  ['請假紀錄', '結束日', 'end_date', '日期', 'YYYY-MM-DD', ''],
  ['請假紀錄', '起始時間', 'start_time', '時間', 'HH:mm（空白=全日）', ''],
  ['請假紀錄', '結束時間', 'end_time', '時間', 'HH:mm（空白=全日）', ''],
  ['請假紀錄', '天數', 'days', '小數', '0.5 / 1 / 2 …', '可半日'],
  ['請假紀錄', '時數', 'hours', '小數', '', '部分日請假用'],
  ['請假紀錄', '事由', 'reason', '字串', '自由輸入', ''],
  ['請假紀錄', '狀態', 'status', '列舉', '待簽核 / 已核准 / 已駁回', ''],
  ['請假紀錄', '簽核人', 'approver_id', '字串', '店長 employee_id', ''],
  ['請假紀錄', '簽核時間', 'approved_at', '時間戳', '', ''],
  ['請假紀錄', '簽核備註', 'approver_comment', '字串', '', ''],
